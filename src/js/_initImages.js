...
window.onscroll = function(){
	for(var i=0;i<document.getElementsByTagName("figure").length;i++){
		...
		if(changeImage.getBoundingClientRect().top<window.innerHeight){
			if(imageSource.slice(0,22)=="https://gdb.rferl.org/"){
        console.log("NOW "+changeImage.children[0].children[0]);
				changeImage.children[1].children[0].setAttribute("src", imageSource.split(".")[0]+"."+imageSource.split(".")[1]+"."+imageSource.split(".")[2]+"_w1200."+imageSource.split(".")[3]);
				}
			};
		};
	};
