// Pangea Apollo Framework 2.0 [Reykjavík]
// " + new Date().getFullYear() + " (c) Pangea Digital, Radio Free Europe/Radio Liberty, Inc.
// All rights reserved

// Main Meta Config for Sites
var siteConfig = {
  af_da: {
    siteBrand: "rfe",
    siteCulture: "rfe-arabic",
		siteName: "Afghan [Dari]",
    siteNameLocal: "رادیو آزادی / رادیوی اروپای آزاد",
    siteURL: "https://da.azadiradio.com/",
    siteLangISO: "prs",
    siteDir: "rtl",
    siteSocialMeta: {
      facebook: 346344192125581,
      twitter: "@RadioAzadi"
    },
    siteLocal: {
      copyright: "همۀ حقوق چاپ و کاپی رایت این سایت برای رادیو آزادی محفوظ است",
      followUs: "ما را تعقیب کنید",
      share: "شریک ساختن"
    },
    siteSocial: {
      facebook: "AzadiR",
      twitter: "DaRadioAzadi",
      youtube: "user/Azadiradio",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: false,
			email: false
		}
  },
  af_en: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Afghan [English]",
    siteNameLocal: "Gandhara RFE/RL",
    siteURL: "https://gandhara.rferl.org/",
    siteLangISO: "en",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 758596314238306,
      twitter: "@GandharaRFE"
    },
    siteLocal: {
      copyright: "Radio Free Europe/Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc. All Rights Reserved.",
      followUs: "Follow Us",
      share: "Share"
    },
    siteSocial: {
      facebook: "GandharaRFE",
      twitter: "GandharaRFE",
      youtube: undefined,
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  af_pa: {
    siteBrand: "rfe",
    siteCulture: "rfe-arabic",
		siteName: "Afghan [Pashto]",
    siteNameLocal: "ازادي راډیو\\دازادې اروپا راډیو",
    siteURL: "https://pa.azadiradio.com/",
    siteLangISO: "ps",
    siteDir: "rtl",
    siteSocialMeta: {
      facebook: 128423907303349,
      twitter: "@pazadio"
    },
    siteLocal: {
      copyright: "د دې سایټ د ټولو مطالبو حقوق له ازادي راډیو سره خوندي دي",
      followUs: "راسره ملګري شئ",
      share: "شريکول"
    },
    siteSocial: {
      facebook: "azadiradio",
      twitter: "pazadiradio",
      youtube: "user/Azadiradio",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: false,
      email: false
		}
  },
  ar: {
    siteBrand: "rfe",
    siteCulture: "rfe-armenian",
		siteName: "Armenian",
    siteNameLocal: "Ազատություն ռ/կ",
    siteURL: "https://www.azatutyun.am/",
    siteLangISO: "hy",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 141126305932755,
      twitter: "@azatutyunradio"
    },
    siteLocal: {
      copyright: "«Ազատ Եվրոպա/Ազատություն» ռադիոկայան © " + new Date().getFullYear() + "",
      followUs: "ՀԵՏԵՎԵՔ ՄԵԶ",
      share: "Կիսվել"
    },
    siteSocial: {
      facebook: "azatutyun",
      twitter: "azatutyunradio",
      youtube: "azatutyunradio",
      vkontakte: "azatutyuntv",
      googlePlus: "u/0/117496679283230849076/posts",
      instagram: "azatutyuntv",
      telegram: undefined,
      odnoklassniki: "azatutyun",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  ar_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Armenian [Russian]",
    siteNameLocal: "Радио Азатутюн",
    siteURL: "https://rus.azatutyun.am/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: "1731694953739871",
      twitter: "@armenialiberty"
    },
    siteLocal: {
      copyright: "Радио Азатутюн © " + new Date().getFullYear() + " RFE/RL, Inc. Все права защищены.",
      followUs: "Социальные сети",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "azatutyunru",
      twitter: "azatutyunradio",
      youtube: "azatutyunradio",
      vkontakte: "azatutyuntv",
      googlePlus: "u/0/117496679283230849076/posts",
      instagram: "azatutyuntv",
      telegram: undefined,
      odnoklassniki: "azatutyun",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  az: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Azeri",
    siteNameLocal: "AzadlıqRadiosu",
    siteURL: "https://www.azadliq.org/",
    siteLangISO: "az",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 115254408502032,
      twitter: "@RadioAzadliq"
    },
    siteLocal: {
      copyright: "Azadlıq Radiosu © " + new Date().getFullYear() + " Inc. | Bütün hüquqlar qorunur",
      followUs: "Bizi izlə",
      share: "Paylaş"
    },
    siteSocial: {
      facebook: "azadliqradiosu",
      twitter: "RadioAzadliq",
      youtube: "user/AzadliqRadiosu",
      vkontakte: undefined,
      googlePlus: "u/0/+azadliqradiosu",
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "radioazadliq",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  be: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Belarus",
    siteNameLocal: "Радыё Свабода. Навіны Беларусі",
    siteURL: "https://www.svaboda.org/",
    siteLangISO: "be",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 117352898288982,
      twitter: "@svaboda"
    },
    siteLocal: {
      copyright: "Радыё Свабода © " + new Date().getFullYear() + " РС | Усе правы захаваныя.",
      followUs: "Сачыце за абнаўленьнямі",
      share: "Падзяліцца"
    },
    siteSocial: {
      facebook: "svaboda.org",
      twitter: "svaboda",
      youtube: "user/svabodavideo",
      vkontakte: "radiosvaboda",
      googlePlus: undefined,
      instagram: "radiosvaboda",
      telegram: "radiosvaboda",
      odnoklassniki: "group/52126350901465",
      livejournal: undefined,
      soundcloud: "svaboda"
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  cr_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Crimea [Russian]",
    siteNameLocal: "Крым.Реалии",
    siteURL: "https://ru.krymr.com/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 283941958441966,
      twitter: "@krymrealii"
    },
    siteLocal: {
      copyright: "© Крым.Реалии, " + new Date().getFullYear() + " | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "krymrealii",
      twitter: "krymrealii",
      youtube: "channel/UCTxc3zFdloSLhqPd6mfIMKA",
      vkontakte: "krymrealii",
      googlePlus: undefined,
      instagram: "krymrealii",
      telegram: undefined,
      odnoklassniki: "group/52237190299809",
      livejournal: undefined,
      soundcloud: "radio-krym-realii"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  cr_tb: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Crimea [Tatar-Bashkir]",
    siteNameLocal: "Qırım.Aqiqat",
    siteURL: "https://ktat.krymr.com/",
    siteLangISO: "crh",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 283941958441966,
      twitter: "@krymrealii"
    },
    siteLocal: {
      copyright: "© Qırım.Aqiqat, " + new Date().getFullYear() + " | All Rights Reserved.",
      followUs: "Qoşulıñız!",
      share: "Paylaşmaq"
    },
    siteSocial: {
      facebook: "krymrealii",
      twitter: "krymrealii",
      youtube: "channel/UCTxc3zFdloSLhqPd6mfIMKA",
      vkontakte: "krymrealii",
      googlePlus: undefined,
      instagram: "krymrealii",
      telegram: undefined,
      odnoklassniki: "group/52237190299809",
      livejournal: undefined,
      soundcloud: "radio-krym-realii"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  cr_uk: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Crimea [Ukrainian]",
    siteNameLocal: "Крим Реалії",
    siteURL: "https://ua.krymr.com/",
    siteLangISO: "uk",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 283941958441966,
      twitter: "@krymrealii"
    },
    siteLocal: {
      copyright: "© Крим.Реалії, " + new Date().getFullYear() + " | Усі права застережено.",
      followUs: "Долучайся",
      share: "Поділитись"
    },
    siteSocial: {
      facebook: "krymrealii",
      twitter: "krymrealii",
      youtube: "channel/UCTxc3zFdloSLhqPd6mfIMKA",
      vkontakte: "krymrealii",
      googlePlus: undefined,
      instagram: "krymrealii",
      telegram: undefined,
      odnoklassniki: "group/52237190299809",
      livejournal: undefined,
      soundcloud: "radio-krym-realii"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  en: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "English",
    siteNameLocal: "Radio Free Europe / Radio Liberty",
    siteURL: "https://www.rferl.org/",
    siteLangISO: "en",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 122264309574,
      twitter: "@RFERL"
    },
    siteLocal: {
      copyright: "Radio Free Europe/Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc. All Rights Reserved.",
      followUs: "Follow Us",
      share: "Share"
    },
    siteSocial: {
      facebook: "rferl",
      twitter: "RFERL",
      youtube: "user/rferlonline",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "rfe.rl",
      telegram: "rferl",
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
	},
  press: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "English Press Room",
    siteNameLocal: "Radio Free Europe / Radio Liberty",
    siteURL: "https://pressroom.rferl.org/",
    siteLangISO: "en",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 122264309574,
      twitter: "@RFERL"
    },
    siteLocal: {
      copyright: "Radio Free Europe/Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc. All Rights Reserved.",
      followUs: "Follow Us",
      share: "Share"
    },
    siteSocial: {
      facebook: "rferl",
      twitter: "RFERLPress",
      youtube: "user/rferlonline",
      vkontakte: undefined,
      googlePlus: "rferl",
      instagram: "rfe.rl",
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  ge: {
    siteBrand: "rfe",
    siteCulture: "rfe-georgian",
		siteName: "Georgian",
    siteNameLocal: "რადიო თავისუფალი ევროპა/რადიო თავისუფლება",
    siteURL: "https://www.radiotavisupleba.ge/",
    siteLangISO: "ka",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 189165972359,
      twitter: "@Rtavisupleba"
    },
    siteLocal: {
      copyright: "რადიო თავისუფლება, RFE/RL, Inc. ყველა უფლება დაცულია",
      followUs: "გამოიწერე",
      share: "გაზიარება"
    },
    siteSocial: {
      facebook: "radiotavisupleba",
      twitter: "Rtavisupleba",
      youtube: "user/RadioTavisupleba",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "tavisupleba",
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  ge_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Georgian [Russian]",
    siteNameLocal: "Эхо Кавказа",
    siteURL: "https://www.ekhokavkaza.com/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 136623139697269,
      twitter: "@ekhokavkaza"
    },
    siteLocal: {
      copyright: "Эхо Кавказа © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "ekhokavkaza",
      twitter: "ekhokavkaza",
      youtube: "channel/UCFV9RoQ52yi6ls3_nIsYb-g",
      vkontakte: "ekhokavkaza",
      googlePlus: "u/0/+Ekhokavkaza",
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  kz: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Kazakh",
    siteNameLocal: "Азат Еуропа / Азаттық Радиосы",
    siteURL: "https://www.azattyq.org/",
    siteLangISO: "kk",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 205061959567731,
      twitter: "@AzattyqRadiosy"
    },
    siteLocal: {
      copyright: "Азат Еуропа / Азаттық радиосы © " + new Date().getFullYear() + ", Inc. | Барлық құқықтары қорғалған",
      followUs: "Жазылыңыз",
      share: "Бөлісу"
    },
    siteSocial: {
      facebook: "azattyq",
      twitter: "AzattyqRadiosy",
      youtube: "user/AzattyqRadio",
      vkontakte: "azattyqradiosy",
      googlePlus: "b/116539418582921678432/+%D0%B0%D0%B7%D0%B0%D1%82%D1%82%D1%8B%D0%BA/posts",
      instagram: "azattyq",
      telegram: undefined,
      odnoklassniki: "azattyq",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  kz_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Kazakh [Russian]",
    siteNameLocal: "Радио Азаттык",
    siteURL: "https://rus.azattyq.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 51124334878,
      twitter: "@Radio_Azattyk"
    },
    siteLocal: {
      copyright: "Радио Азаттык © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "RadioAzattyq",
      twitter: "Radio_Azattyk",
      youtube: "user/AzattyqRadio",
      vkontakte: "radioazattyq",
      googlePlus: "b/116539418582921678432/+%D0%B0%D0%B7%D0%B0%D1%82%D1%82%D1%8B%D0%BA/posts",
      instagram: "azattyq",
      telegram: undefined,
      odnoklassniki: "azattyq",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: false,
			email: false
		}
  },
  al: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Kosovo",
    siteNameLocal: "Radio Evropa e Lirë",
    siteURL: "https://www.evropaelire.org/",
    siteLangISO: "sq",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 299620092248,
      twitter: "@RFE_Kosova"
    },
    siteLocal: {
      copyright: "Radio Evropa e Lirë © " + new Date().getFullYear() + " RFE/RL, Inc. Të gjitha të drejtat e rezervuara.",
      followUs: "Na ndiqni",
      share: "Shpërndaje"
    },
    siteSocial: {
      facebook: "evropaelire",
      twitter: "RFE_Kosova",
      youtube: "user/RadioEvropaLire",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "radioevropaelire",
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  kg: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Kyrgyz",
    siteNameLocal: "Азаттык Υналгысы",
    siteURL: "https://www.azattyk.org/",
    siteLangISO: "ky",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 132515023525838,
      twitter: "@Azattyk_Radiosu"
    },
    siteLocal: {
      copyright: "Radio Evropa e Lirë © " + new Date().getFullYear() + " RFE/RL, Inc. Të gjitha të drejtat e rezervuara.",
      followUs: "Онлайн шерине",
      share: "Бөлүшүңүз"
    },
    siteSocial: {
      facebook: "Azattyk.unalgysy",
      twitter: "Azattyk_Radiosu",
      youtube: "user/AzattykUnalgysy",
      vkontakte: "azattykkg",
      googlePlus: undefined,
      instagram: "azattyk.kg",
      telegram: undefined,
      odnoklassniki: "azattyk",
      livejournal: undefined,
      soundcloud: "azattyk-audio"
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  kg_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Kyrgyz [Russian]",
    siteNameLocal: "Радио Азаттык",
    siteURL: "https://rus.azattyk.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 220634991334437,
      twitter: "@AzattykMedia"
    },
    siteLocal: {
      copyright: "Радио Азаттык (Кыргызская служба РСЕ/РС) © Радио Свободная Европа/Радио Свобода. Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "azattyk",
      twitter: "AzattykMedia",
      youtube: "user/AzattykUnalgysy",
      vkontakte: "azattykkg",
      googlePlus: undefined,
      instagram: "azattykmedia",
      telegram: "azattykrus",
      odnoklassniki: "azattyk",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  mk: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Macedonian",
    siteNameLocal: "Радио Слободна Eвропа",
    siteURL: "https://www.slobodnaevropa.mk/",
    siteLangISO: "mk",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 97222983724,
      twitter: "RSE_makedonski"
    },
    siteLocal: {
      copyright: "Радио Слободна Европа / Радио Слобода. Сите права се резервирани",
      followUs: "Следете не",
      share: "Сподели"
    },
    siteSocial: {
      facebook: "RSEMK",
      twitter: "RSE_makedonski",
      youtube: "user/SlobodnaEvropa",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "generacija.z",
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: true
		}
  },
  mo: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Moldovan",
    siteNameLocal: "Europa Liberă",
    siteURL: "https://www.europalibera.org/",
    siteLangISO: "ro",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 182173838166,
      twitter: undefined
    },
    siteLocal: {
      copyright: "Radio Free Europe/Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc. Toate drepturile rezervate",
      followUs: "Urmărește-ne",
      share: "Distribuie"
    },
    siteSocial: {
      facebook: "europalibera.org",
      twitter: undefined,
      youtube: "user/europalibera11",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "europalibera",
      telegram: undefined,
      odnoklassniki: "profile/464250734781",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  nc_ce: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "North Caucasus [Chechen]",
    siteNameLocal: "Маршо Радио",
    siteURL: "https://www.radiomarsho.com/",
    siteLangISO: "ce",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 203479823160953,
      twitter: "@radiomarsho"
    },
    siteLocal: {
      copyright: "Маршо Радио © " + new Date().getFullYear() + " RFE/RL, Inc. Ерриг бакъонаш долаяьхна ю",
      followUs: "Лаха тхо",
      share: "Яхьийта"
    },
    siteSocial: {
      facebook: "radiomarsho",
      twitter: "radiomarsho",
      youtube: "user/arsno10",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: false,
			email: false
		}
  },
  nc_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "North Caucasus [Russian]",
    siteNameLocal: "Кавказ.Реалии",
    siteURL: "https://www.kavkazr.com/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 152873214901474,
      twitter: "@kavkaz_realii"
    },
    siteLocal: {
      copyright: "© Кавказ.Реалии, " + new Date().getFullYear() + " | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "KavkazRealii",
      twitter: "kavkaz_realii",
      youtube: "channel/UCa0Vkb4iSyFGyy8arlz7MCQ",
      vkontakte: "kavkaz_realii",
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "kavkaz.realii",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: false,
      email: false
		}
  },
  pk: {
    siteBrand: "rfe",
    siteCulture: "rfe-arabic",
		siteName: "Pakistani",
    siteNameLocal: "مشال راډیو",
    siteURL: "https://www.mashaalradio.com/",
    siteLangISO: "ur",
    siteDir: "rtl",
    siteSocialMeta: {
      facebook: 283201628374188,
      twitter: "@Mashaalradio"
    },
    siteLocal: {
      copyright: "د دې سایټ دټولومطالبوحقوق له مشال راډیوسره خوندي دي",
      followUs: "موږ وڅارئ",
      share: "شریک کړئ"
    },
    siteSocial: {
      facebook: "mashaalradio",
      twitter: "Mashaalradio",
      youtube: "user/mashaalradio",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  fa: {
    siteBrand: "rfe",
    siteCulture: "rfe-arabic",
		siteName: "Radio Farda",
    siteNameLocal: "رادیو فردا",
    siteURL: "https://www.radiofarda.com/",
    siteLangISO: "fa",
    siteDir: "rtl",
    siteSocialMeta: {
      facebook: 83359068840,
      twitter: "@RadioFarda_"
    },
    siteLocal: {
      copyright: "© " + new Date().getFullYear() + " تمام حقوق این وب‌سایت، بر اساس مقررات کپی‌رایت، برای رادیو فردا محفوظ است.",
      followUs: "به ما بپیوندید",
      share: "ارسال"
    },
    siteSocial: {
      facebook: "radio.farda",
      twitter: "radiofarda_",
      youtube: "user/TheRadiofarda",
      vkontakte: undefined,
      googlePlus: "https://plus.google.com/+radiofarda/posts",
      instagram: "radiofarda",
      telegram: "radiofarda",
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: "radio-farda"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  fa_en: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Radio Farda [English]",
    siteNameLocal: "Radio Farda English Site",
    siteURL: "https://en.radiofarda.com/",
    siteLangISO: "en",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: undefined,
      twitter: "@RadioFarda_Eng"
    },
    siteLocal: {
      copyright: "Radio Free Europe/Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc. All Rights Reserved.",
      followUs: "Follow Us",
      share: "Share"
    },
    siteSocial: {
      facebook: undefined,
      twitter: "RadioFarda_Eng",
      youtube: undefined,
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: false,
      email: false
		}
  },
  ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Russian",
    siteNameLocal: "Радио Свобода",
    siteURL: "https://www.svoboda.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 177288837553,
      twitter: "@SvobodaRadio"
    },
    siteLocal: {
      copyright: "Радио Свобода © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "radiosvoboda",
      twitter: "SvobodaRadio",
      youtube: "user/SvobodaRadio",
      vkontakte: "public12637912",
      googlePlus: undefined,
      instagram: "radiosvoboda",
      telegram: "radiosvoboda",
      odnoklassniki: "radio.svoboda",
      livejournal: "http://svobodaradio.livejournal.com/",
      soundcloud: "radio-svoboda"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  sib: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Sibreal",
    siteNameLocal: "Сибирь.Реалии",
    siteURL: "https://www.sibreal.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: "2178891482337491",
      twitter: "@sibrealii"
    },
    siteLocal: {
      copyright: "Сибирь.Реалии © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "sibrealii",
      twitter: "sibrealii",
      youtube: undefined,
      vkontakte: "sibrealii",
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "group/53813651439688",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: false,
      podcast: false,
      email: false
		}
  },
  sh: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "South Slavic",
    siteNameLocal: "Radio Slobodna Evropa",
    siteURL: "https://www.slobodnaevropa.org/",
    siteLangISO: "sh",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 196390633882,
      twitter: "@RSE_Balkan"
    },
    siteLocal: {
      copyright: "Sva prava zadržana. Radio Free Europe / Radio Liberty © " + new Date().getFullYear() + " RFE/RL, Inc.",
      followUs: "Pratite nas",
      share: "Podijelite"
    },
    siteSocial: {
      facebook: "RSE.Balkan",
      twitter: "RSE_Balkan",
      youtube: "user/RadioSlobodnaEvropa",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "radioslobodnaevropa",
      telegram: undefined,
      odnoklassniki: undefined,
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  tj: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Tajik",
    siteNameLocal: "Радиои Озодӣ",
    siteURL: "https://www.ozodi.org/",
    siteLangISO: "tg",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 174906359256843,
      twitter: "@RadioiOzodi"
    },
    siteLocal: {
      copyright: "Радиои Аврупои Озод / Радиои Озодӣ © " + new Date().getFullYear() + " RFE/RL. Inc. Ҳамаи ҳуқуқ маҳфуз аст.",
      followUs: "Пайгирӣ кунед",
      share: "Пахш"
    },
    siteSocial: {
      facebook: "radio.ozodi",
      twitter: "RadioiOzodi",
      youtube: "user/Ozodivideo",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "radioiozodi",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  tj_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Tajik [Russian]",
    siteNameLocal: "Радио Озоди",
    siteURL: "https://rus.ozodi.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 174906359256843,
      twitter: "@RadioiOzodi"
    },
    siteLocal: {
      copyright: "TAJIK RU © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "radio.ozodi",
      twitter: "RadioiOzodi",
      youtube: "user/Ozodivideo",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "radioiozodi",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: false
		}
  },
  tb: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Tatar-Bashkir",
    siteNameLocal: "Азатлык Радиосы",
    siteURL: "https://www.azatliq.org/",
    siteLangISO: "tt",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 139505469404757,
      twitter: "@Azatliq"
    },
    siteLocal: {
      copyright: "Азатлык Радиосы © " + new Date().getFullYear() + " RFE/RL, Inc. | Бар хокуклар сакланган",
      followUs: "Безгә кушылыгыз!",
      share: "Уртаклаш"
    },
    siteSocial: {
      facebook: "azatliq",
      twitter: "Azatliq",
      youtube: "user/azatliqradiosi",
      vkontakte: "azatliq_radiosi",
      googlePlus: undefined,
      instagram: "azatliq",
      telegram: "joinchat/CxW5dz9ILszGJNLz9gk_RQ",
      odnoklassniki: "group/54630774538240",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: false,
      email: false
		}
  },
  tb_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Tatar-Bashkir [Russian]",
    siteNameLocal: "Idel.Реалии",
    siteURL: "https://www.idelreal.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 307579732907195,
      twitter: "@Idel_Realii"
    },
    siteLocal: {
      copyright: "Idel.Реалии © " + new Date().getFullYear() + " RFE/RL, Inc. Все права защищены",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "Idel.Realii",
      twitter: "Idel_Realii",
      youtube: "channel/UCiyr8eopCxMT5YBZlK7-v3A",
      vkontakte: "idel.real",
      googlePlus: undefined,
      instagram: "idel.realii",
      telegram: "joinchat/Dn0npUCnnm3MHeCj1xlD1w",
      odnoklassniki: "group/54104566005878",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
      podcast: false,
      email: false
		}
  },
  tu: {
    siteBrand: "rfe",
    siteCulture: "rfe-latin",
		siteName: "Turkmen",
    siteNameLocal: "Azat Ýewropa/Azatlyk Radiosy",
    siteURL: "https://www.azathabar.com/",
    siteLangISO: "tk",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 169702108452,
      twitter: "@azathabar"
    },
    siteLocal: {
      copyright: "Azat Ýewropa/Azatlyk Radiosy © " + new Date().getFullYear() + " RFE/RL, Inc. Ähli hukuklar goralan.",
      followUs: "Bizi yzarlaň",
      share: "Paýlaş"
    },
    siteSocial: {
      facebook: "azatlykradiosy",
      twitter: "azathabar",
      youtube: "user/azatlykonline",
      vkontakte: "azathabar",
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "radioazatlyk",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: true,
			email: false
		}
  },
  tu_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Turkmen [Russian]",
    siteNameLocal: "Радио Азатлык",
    siteURL: "https://rus.azathabar.com/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 169702108452,
      twitter: "@azathabar"
    },
    siteLocal: {
      copyright: "Радио Азатлык © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "azatlykradiosy",
      twitter: "azathabar",
      youtube: "user/azatlykonline",
      vkontakte: "radioazatlyk",
      googlePlus: undefined,
      instagram: undefined,
      telegram: undefined,
      odnoklassniki: "radioazatlyk",
      livejournal: undefined,
      soundcloud: undefined
		},
		siteServices: {
			rss: true,
      podcast: false,
      email: true
		}
  },
  ua: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Ukrainian",
    siteNameLocal: "Радіо Свобода",
    siteURL: "https://www.radiosvoboda.org/",
    siteLangISO: "uk",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 359429071842,
      twitter: "@radiosvoboda"
    },
    siteLocal: {
      copyright: "© Радіо Свобода, " + new Date().getFullYear() + " | Усі права застережено.",
      followUs: "Долучайся!",
      share: "Поділитись"
    },
    siteSocial: {
      facebook: "RadioSvoboda.Org",
      twitter: "radiosvoboda",
      youtube: "user/RadioSvobodaOrg?sub_confirmation=1",
      vkontakte: "club18625777",
      googlePlus: "u/0/+radiosvoboda/posts",
      instagram: "radio.svoboda",
      telegram: undefined,
      odnoklassniki: "group/52030404493473",
      livejournal: undefined,
      soundcloud: "radiosvoboda"
    },
		siteServices: {
			rss: true,
      podcast: true,
      email: true
		}
  },
  uz: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Uzbek",
    siteNameLocal: "Озодлик радиоси",
    siteURL: "https://www.ozodlik.org/",
    siteLangISO: "uz",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 88521376958,
      twitter: "@ozodlik"
    },
    siteLocal: {
      copyright: "Озодлик радиоси © " + new Date().getFullYear() + " RFE/RL, Inc. | Барча ҳуқуқлар ҳимояланган.",
      followUs: "Социал Тармоқлар",
      share: "Ўртоқлашинг"
    },
    siteSocial: {
      facebook: "ozodlikradiosi",
      twitter: "ozodlik",
      youtube: "user/Ozodlikradiosi",
      vkontakte: undefined,
      googlePlus: "u/0/113391318787175877177/posts",
      instagram: "ozodlikradiosi",
      telegram: "joinchat/AAAAADwn2PokiMCcyNsWLw",
      odnoklassniki: "ozodlikradiosi",
      livejournal: undefined,
      soundcloud: "ozodlikradiosi"
    },
		siteServices: {
			rss: false,
      podcast: true,
      email: false
		}
  },
  uz_ru: {
    siteBrand: "rfe",
    siteCulture: "rfe-cyrillic",
		siteName: "Uzbek [Russian]",
    siteNameLocal: "Радио Озодлик",
    siteURL: "https://rus.ozodlik.org/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: 180344302346183,
      twitter: "@ozodlik"
    },
    siteLocal: {
      copyright: "Радио Озодлик © " + new Date().getFullYear() + " RFE/RL, Inc. | Все права защищены.",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "Ozodlik",
      twitter: "ozodlik",
      youtube: "user/Ozodlikradiosi",
      vkontakte: undefined,
      googlePlus: undefined,
      instagram: "radioozodlik",
      telegram: undefined,
      odnoklassniki: "ozodlikradiosi",
      livejournal: undefined,
      soundcloud: "ozodlikradiosi"
    },
		siteServices: {
			rss: true,
			podcast: false,
			email: false
		}
  },
  ct: {
    siteBrand: "ct",
    siteCulture: "ct-cyrillic",
		siteName: "Current Time",
    siteNameLocal: "Настоящее Время",
    siteURL: "https://www.currenttime.tv/",
    siteLangISO: "ru",
    siteDir: "ltr",
    siteSocialMeta: {
      facebook: "1543950879153281",
      twitter: "@CurrentTimeTv"
    },
    siteLocal: {
      copyright: "Настоящее Время © Все права защищены",
      followUs: "Присоединяйтесь!",
      share: "Поделиться"
    },
    siteSocial: {
      facebook: "currenttimetv",
      twitter: "CurrentTimeTv",
      youtube: "user/currenttimetv",
      vkontakte: "currenttime",
      googlePlus: undefined,
      instagram: "currenttime.tv",
      telegram: "currenttime",
      odnoklassniki: "currenttime",
      livejournal: undefined,
      soundcloud: undefined
    },
		siteServices: {
			rss: true,
			podcast: false,
			email: false
		}
  }
}

// Themes for Favicons
var faviconConfig = {
	ct: "#5bbad5",
	rfe: "#ea6903"
}
